...
const PriceSchemeManagementV2: React.FC = () => {
  ...
  const handleOperatorChange = (id: string, operator: string) => {
    setDroppedFields(prev =>
      prev.map(field => {
        ...
        // 如果操作符是区间类型或特殊操作符，设置为相应的模式
        if (operator === '数值区间' || operator === '日期区间') {
          newField.isRange = true;
          newField.value2 = '';
        } else if (operator === '为空' || operator === '不为空') {
          newField.isRange = true;
          newField.value = '';
          newField.value2 = '';
        } else {
          newField.isRange = false;
          delete newField.value2;
        }
   ...
   * @param id 字段ID
   * @param value 值
   * @param isSecondValue 是否是第二个值（用于区间）
   */
  const handleValueChange = (id: string, value: string, isSecondValue: boolean = false) => {
    setDroppedFields(prev =>
      prev.map(field => {
        if (field.id === id) {
          if (isSecondValue) {
            return { ...field, value2: value };
          } else {
            return { ...field, value };
          }
        }
                ...
                <Row gutter={[16, 16]} style={{ marginTop: '24px' }}>
                  <Col span={24}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <span style={{ width: '80px', textAlign: 'right', marginRight: '8px' }}>比对对象:</span>
        ...
        onOk={() => {
          ...
          const newRanges = droppedFields.map(field => {
            ...
            switch (field.operator) {
              ...
              case '包含':
              case '不包含':
                conditionText = field.operator;
                values = [field.value];
                break;
              case '数值区间':
              case '日期区间':
                conditionText = '区间';
                values = [field.value, field.value2 || ''];
                break;
              default:
                conditionText = field.operator;
                values = [field.value];
              ...
              >
                <div style={{
                  ...
                  {[
                    ...
                    { field: 'volume', name: '体积', type: 'number' }
                  ].map((field, index) => (
                    <div
                      key={index}
                      draggable
              ...
              >
                <div style={{
                  ...
                  {[
                    ...
                    { field: 'create_time', name: '创建时间', type: 'date' },
                    { field: 'update_time', name: '更新时间', type: 'date' },
                    { field: 'delivery_date', name: '交货日期', type: 'date' },
                    { field: 'start_date', name: '开始日期', type: 'date' },
                    { field: 'end_date', name: '结束日期', type: 'date' }
                  ].map((field, index) => (
                    <div
                      key={index}
                      draggable
                  ...
                  <Table
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        key: 'operator',
                        width: 140,
                        render: (operator, record: any) => {
                          const operators = getOperatorsByType(record.type);
                          return (
                            <Select
                              value={operator}
                              onChange={(value) => handleOperatorChange(record.id, value)}
                              style={{ width: '100%' }}
                              size="small"
                            >
                              {operators.map(op => (
                      ...
                      },
                      {
                        ...
                        render: (_, record: any) => {
                          // 处理特殊操作符（为空、不为空）
                          if (record.operator === '为空' || record.operator === '不为空') {
                            return (
                              <Tag color="blue" style={{ fontSize: '12px' }}>
                                {record.operator === '为空' ? '值为空' : '值不为空'}
                              </Tag>
                            );
                          }

                          // 处理正则匹配操作符
                          if (record.operator === '正则匹配') {
                            return (
                              <Input
                                ...
                                {record.type === 'date' ? (
                                  <>
                                    <DatePicker
                                      ...
                                      onChange={(date) => handleValueChange(record.id, date ? date.format('YYYY-MM-DD') : '')}
                                      size="small"
                                      style={{ width: '140px' }}
                                    />
                                    ...
                                    <DatePicker
                                      placeholder="结束日期"
                                      value={record.value2 ? dayjs(record.value2) : null}
                                      onChange={(date) => handleValueChange(record.id, date ? date.format('YYYY-MM-DD') : '', true)}
                                      size="small"
                                      style={{ width: '140px' }}
                                    />
                                  </>
                                  ...
                                  <>
                                    <InputNumber
                                      ...
                                      onChange={(value) => handleValueChange(record.id, value?.toString() || '')}
                                      size="small"
                                      style={{ width: '100px' }}
                                    />
                                    <span style={{ color: '#999' }}>至</span>
                                    <InputNumber
                                      placeholder="最大值"
                                      value={record.value2 ? Number(record.value2) : null}
                                      onChange={(value) => handleValueChange(record.id, value?.toString() || '', true)}
                                      size="small"
                                      style={{ width: '100px' }}
                                    />
                                  </>
